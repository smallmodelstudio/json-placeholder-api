...
describe('Todos (e2e)', () => {
  ...
  describe('GET /todos', () => {
    it('returns the list of todos from upstream', async () => {
      ...
      const response = await api(app).get('/todos').expect(200);
    ...
    it('forwards ?userId= as an upstream query param', async () => {
      ...
      const response = await api(app).get('/todos?userId=7').expect(200);
  ...
  describe('GET /todos/:id', () => {
    it('returns a single todo from upstream', async () => {
      ...
      const response = await api(app).get('/todos/1').expect(200);
    ...
    it('rejects a non-positive-integer id with 400', async () => {
      await api(app).get('/todos/abc').expect(400);
    });
  });
  ...
  describe('POST /todos', () => {
    it('creates a todo and returns the upstream response', async () => {
      ...
      const response = await api(app).post('/todos').send(dto).expect(201);
  ...
  describe('PUT /todos/:id', () => {
    it('replaces a todo and returns the upstream response', async () => {
      ...
      const response = await api(app).put('/todos/1').send(dto).expect(200);
  ...
  describe('PATCH /todos/:id', () => {
    it('partially updates a todo and returns the upstream response', async () => {
      ...
      const response = await api(app).patch('/todos/1').send(dto).expect(200);
  ...
  describe('DELETE /todos/:id', () => {
    it('deletes a todo and returns the upstream response', async () => {
      ...
      const response = await api(app).delete('/todos/1').expect(200);
